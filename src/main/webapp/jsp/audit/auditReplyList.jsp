					...
					<div class="block">
						<div class="navbar navbar-inner block-header" style="border-bottom: none;">
							<form class="form-inline" id="myForm" name="myForm"
							      action="${BasePath}/audit/replyList?${PublicParams}" method="post">
								...
								<c:forEach items="${topicReplyList}" var="obj">
									<tr>
                                        ...
                                        <td>
                                            ...
                                            标题:<a
                                                href="${BasePath}/topic/topicDetail?topicId=${obj.topicId}&type=${type}&itemId=${itemId}"
										...
										<pe:permission key="banUser">
											<td style="vertical-align:middle">
											  <button class="btn btn-danger btn-small" onclick="openModel('${BasePath}/audit/toBanUser?uid=${obj.uid}&replyId=${obj.id}&nick=${obj.nick}','封禁层主','45%','70%')">封禁层主</button>
											</td>
										</pe:permission>
									</tr>
								</c:forEach>
...
<script type="text/javascript">
	...
	function passAudit() {
		...
		$.ajax({
			type: "POST",
			url: "${BasePath}/audit/passReply?ids=" + ids,
			datatype: "json",
	...
	function deleteAudit() {
		...
		if (confirm("确定要删除吗？")) {
			$.ajax({
				type: "POST",
				url: "${BasePath}/audit/deleteReply?ids=" + ids,
				datatype: "json",
				success: function (data) {
					if (data.errno == 1) {
						alert(data.errdesc);
    ...
    function revokeDelelt() {
        ...
        $.ajax({
            type: "POST",
            url: "${BasePath}/audit/revokeDelelt?replyIds=" + ids +"&topicIds=",
            datatype: "json",
            success: function (data) {
                if (data.errno == 1) {
                    alert(data.errdesc);
    ...
    function hiddenReply(digest) {
        ...
        var path = "${BasePath}/topic/hiddenReply?replyIds=" + ids + "&digest=" + digest;
        qqAjax(path);
    }
    ...
    function ignoreReply(){
        ...
        var path = "${BasePath}/audit/ignoreReply?replyIds=" + ids;
        qqAjax(path);
    }
