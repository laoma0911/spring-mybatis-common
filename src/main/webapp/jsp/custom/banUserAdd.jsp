									...
									<form class="form-horizontal" id="saveCustom" name="saveCustom"  action="${BasePath }/custom/save" method="post">
										...
										<fieldset>
											...
											<div class="control-group">
												...
												<div class="controls">
												  <input class="input-xlarge focused" name="uids" id="uids"   type="text" />
												</div>
                                            </div>
											  <div class="control-group">
...
function saveCustom11(){
	...
	$("#saveCustom").submit(function() {
		...
		var path="${BasePath }/custom/banUserSave?uids="+device+"&period="+period+"&remark="+remark;
		path=encodeURI(path);
		path=encodeURI(path); 
	  $.ajax({
          type:"POST",
          ...
          success:function(data){
        	  if(data.errno==1){
        		  ...
        		  $("#errodesc").html(data.errdesc);
       			 ...
       			 window.location.href="${BasePath}/custom/banUserList?type=${type}&itemId=${itemId}";
			 }
          } ,
          error: function(){
            alert("操作出错，请联系管理员！");
