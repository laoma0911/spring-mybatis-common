...
<div class="container-fluid">
         <div class="block"  >
					<div class="navbar navbar-inner block-header"  style="border-bottom: none;" >
 					<form action="${BasePath}/custom/toDeviceList" method="post" id="myform" name="myform">
                  ...
                  <c:forEach items="${userDeviceList}" var="obj">
                    <tr>
                      <td > <a href="${BasePath }/custom/userList?type=4&itemId=20&uid=${obj.uid}"  target="_black">${obj.uid}</a></td>
                        <td>${obj.nick}</td>
                      <td><a href="${BasePath}/custom/toDeviceList?device=${obj.device }&uid="  target="_black" >${obj.device }</a></td>
                      <td>
                      ...
                      <td>
                        ...
                        <td>
                        	<c:choose>
                         	 ...
                         	 <a class="btn btn-small btn-success"  href="${BasePath}/custom/toHistory?uid=${obj.uid}&device=${obj.device }">历史</a></td>
                    </tr>
                  </c:forEach>
                </tbody>
              </table>
...
<script>
	...
	function deleteDevice(device,uid){
		if(!confirm('确定解封该条记录吗？')){
	      ...
	      $.post('${BasePath}/custom/banDeviceDelete',{ 'device':device,'uid':uid },function(data){
	        var obj = eval(data);
	        if(obj.errno==0){
	          alert('操作成功');
	...
	function saveCustom11(){
		...
		$("#saveCustom").submit(function() {
			...
			var path="${BasePath }/custom/banDeviceSave?device="+device+"&period="+period+"&remark="+remark+"&uid="+uid;
			path=encodeURI(path);
			path=encodeURI(path); 
		  $.ajax({
	          type:"POST",
