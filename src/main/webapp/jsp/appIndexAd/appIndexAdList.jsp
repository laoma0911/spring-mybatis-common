                ...
                <div class="span12">
                    <div class="block">
                        <div class="navbar navbar-inner block-header" style="border-bottom: none;">
                            <form id="myForm" name="myForm" action="${BasePath}/appindexad/list" method="post"
                                  class="form-inline">
                                时间：
                        ...
                        <div class="pull-right">
                            <pe:permission key="add">
                                <button class="btn btn-success btn-small" style="margin-top: -5px"
                                        onclick="openModel('${BasePath}/appindexad/toAdd','新增')">新增
                                </button>
                            </pe:permission>
                        </div>
                    </div>
                                        ...
                                        <td>
                                            <pe:permission key="update">
                                                <a class="btn btn-success btn-small"
                                                   href="javascript:openModel('${BasePath}/appindexad/toEdit?id=${obj.id}','修改')">修改</a>
                                            </pe:permission>
                                            <pe:permission key="delete">
                                                <button class="btn del btn-danger btn-small">删除</button>
                                            </pe:permission>
                                     <a class="btn btn-success btn-small" href="javascript:openModel('${BasePath}/appindexad/logList?id=${obj.id}','查看每日')" >查看每日</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
...
<script>
    $(function () {
        ...
        $("input[name='id']").each(function (){
            var id=this.value;
            $.post('${BasePath}/appindexad/getView',{ 'id':id },function(data){
                var obj = eval(data);
                if(obj.errno==0){
                    var html="总量:";
        ...
        $("input[name='id1']").each(function () {
            var id = this.value;
            $.post('${BasePath}/appindexad/getClickNum', {'id': id}, function (data) {
                var obj = eval(data);
                if (obj.errno == 0) {
        ...
        $(".del").bind('click', function () {
            ...
            $.post('${BasePath}/appindexad/delete', {
                'id': uid
            }, function (data) {
                var obj = eval(data);
                if (obj.errno == 0) {
