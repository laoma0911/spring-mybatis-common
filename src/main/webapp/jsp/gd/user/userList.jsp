				...
				<div class="span12">
					...
					<div class="block">
						<div class="navbar navbar-inner block-header"
							...
							<form 	action="${BasePath }/meiyue/user/list?type=${type}&itemId=${itemId}" 	method="post" id="myform" name="myform">
				...
				<div class="block">
					<div class="navbar navbar-inner block-header">
						...
						<pe:permission key="add">
							<button class="btn btn-small btn-success"   onclick="openModel('${BasePath}/meiyue/user/toUserAdd','新增','50%','50%')">新增</button>
						</pe:permission>
									...
									<c:forEach items="${userList}" var="obj">
										<tr>
											...
											<c:if test="${obj.nick!=null && obj.nick!=''}">
                                                <a href="${BasePath}/meiyue/user/nickLogList?type=4&itemId=21&uid=${obj.uid}"  target="_blank" >${obj.nick }</a><br>
                                            </c:if>
                                                <script type="text/javascript">
											...
											<td style="width: 5%;">
												<c:if test="${obj.isMeiyue==1 }">美月</c:if>
												<c:if test="${obj.isMeiyue==0 }">大姨吗</c:if>
											</td>
											...
											<td style="width: 10%;">
												<c:choose>
														<c:when test="${obj.expires==null }">
															...
															<button class="btn btn-success btn-small" onclick="openModel('${BasePath}/meiyue/user/toBanUser?uid=${obj.uid }','封禁用户','50%','50%')">封禁</button>
															</pe:permission>
															<pe:permission key="banUserDev">
															<button class="btn btn-danger btn-small" onclick="openModel('${BasePath}/meiyue/user/toBanUserDev?uid=${obj.uid}','带设备封禁','50%','50%')">带设备封禁</button>
															</pe:permission>
														</c:when>
														<c:otherwise>
															...
															<pe:permission key="delBanUserDev">
																<button class="btn btn-danger btn-small" onclick="openModel('${BasePath}/meiyue/user/toDelUserDev?uid=${obj.uid }','解除用户和设备封禁')">解除用户和设备封禁</button>
															</pe:permission>
														</c:otherwise>
												</c:choose>
												<pe:permission key="lookDevice">
													<a class="btn btn-success btn-small" href="${BasePath}/meiyue/user/toDeviceList?uid=${obj.uid }" target="_blank" >查看设备</a>
												</pe:permission>
												<pe:permission key="update">
													<button class="btn btn-success btn-small" onclick="openModel('${BasePath}/meiyue/user/toUpdateMedal?uid=${obj.uid }','编辑勋章','50%','50%')">编辑勋章</button>
												</pe:permission>
                                                ...
                                                <pe:permission key="setPass">
                                                    <button class="btn btn-success btn-small" onclick="openModel('${BasePath}/meiyue/user/toUpdatePass?uid=${obj.uid }','编辑用户','50%','50%')">编辑用户</button>
                                                </pe:permission>
...
<script>
    function delEmail(uid){
        ...
        $.post('${BasePath}/meiyue/user/delEmail',{ 'uid':uid },function(data){
            var obj = eval(data);
            if(obj.errno==0){
                alert('操作成功');
    ...
    function delMobile(uid){
        ...
        $.post('${BasePath}/meiyue/user/delMobile',{ 'uid':uid },function(data){
            var obj = eval(data);
            if(obj.errno==0){
                alert('操作成功');
                window.location.reload();
	...
	function toUpdate(uid, nick, group_id, sex, type, itemId) {
		...
		nick = encodeURI(nick);
		nick = encodeURI(nick);
		window.location = "${BasePath}/meiyue/user/toUpdate?uid=" + uid + "&nick="
	...
	function deleteDevice(uid){
		if(!confirm('确定解封该条记录吗？')){
	      ...
	      $.post('${BasePath}/meiyue/user/banUserDelete',{ 'uid':uid },function(data){
	        var obj = eval(data);
	        if(obj.errno==0){
	          alert('操作成功');
	          window.location.reload();
	...
	function delUserAvatar(uid){
		if(confirm('确定删除用户头像吗？')){
	      $.post('${BasePath}/meiyue/user/delUserAvatar',{ 'uid':uid },function(data){
	        var obj = eval(data);
	        if(obj.errno==0){
	          alert('操作成功');
	          window.location.reload();
