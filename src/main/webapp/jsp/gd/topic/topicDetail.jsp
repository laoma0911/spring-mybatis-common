    ...
    <div class="row-fluid">
        ...
        <div class="span15" id="content">
            <div class="row-fluid">
                        ...
                        <div class="navbar navbar-inner block-header" style="border-bottom: none;">
                            ...
                            <%--<pe:permission key="reply">--%>
                                <a class="btn cancel btn btn-primary btn-small"
                                   href="javascript:openModel('${BasePath}/meiyue/topic/toReply?replyId=${reply.id}&sign=0&topicId=${topic.id}','回复','50%','65%')">回复</a>
                            <%--</pe:permission>--%>
                            ...
                            <c:if test="${headPic != null}">
                                <a href="${headPic.oriPic}" rel="lightbox[ostec]">
                                    <img id="${headPic.id}" src="${headPic.oriPic}" width="100px" height="100px">
                                </a>
                            </c:if>
                ...
                <div class="block">
                    <div class="navbar navbar-inner block-header" style="border-bottom: none;">
                        <%--<form id="saveCustom" name="saveCustom" action="${BasePath}/meiyue/topic/exportTopicReplys">--%>
                            回复操作：
                        ...
                        <div class="span12">
                            <table class="table table-hover">
                                ...
                                <c:forEach items="${replyResult}" var="reply">
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="replyId" id="replyId" value="${reply.id}_${reply.uid}_${reply.status}"/>
                                        </td>
                                        <td>
                                            ${reply.id}
                                            <br>
                                        ...
                                        <td style="vertical-align:middle">
                                            ...
                                            <%--<pe:permission key="reply">--%>
                                                <a class="btn cancel btn btn-primary btn-small"
                                                   href="javascript:openModel('${BasePath}/meiyue/topic/toReply?replyId=${reply.id}&sign=0&topicId=${reply.topicId}','回复','50%','65%')">回复</a><br>
                                            <%--</pe:permission>--%>
                                            <%--<pe:permission key="like">--%>
                                                <input class="btn cancel btn btn-primary btn-small" type="button" value="点赞"
                                                       onclick="openModel('${BasePath}/meiyue/topic/toReplyLike?replyId=${reply.replyId }&topicId=${reply.topicId }&uid=${reply.uid }','点赞','60%','60%')"/><br>
                                            <%--</pe:permission>--%>
...
<script>
    ...
    function toSort(){
        var sort = $("#sort").val();
        window.location.href="${BasePath}/meiyue/topic/topicDetail?topicId=${topic.id}&sort="+sort+"&type=${type}&itemId=${itemId}";
    }
    ...
    function deleteAll() {
        ...
        if (confirm("确定要删除吗？")) {
            var path = "${BasePath}/meiyue/topic/deleteAll?replyIds=" + replyIdArry + "&type=${type}&itemId=${itemId}";
            $.ajax({
                type: "POST",
                url: path,
                datatype: "json",
    ...
    function cancelAll() {
        ...
        var path = "${BasePath}/meiyue/topic/recoverReply?params=" + replyIdArry + "&type=${type}&itemId=${itemId}";
        $.ajax({
            type: "POST",
            url: path,
            datatype: "json",
    ...
    function deleteReply(replyIds) {
        var path = "${BasePath}/meiyue/topic/deleteAll?replyIds=" + replyIds + "&type=${type}&itemId=${itemId}";
        $.ajax({
            type: "POST",
            url: path,
            datatype: "json",
            success: function (data) {
                if (data.errno == 1) {
                    ...
                    window.location.href = "${BasePath}/meiyue/topic/topicDetail?topicId=${topic.id}&type=${type}&itemId=${itemId}";
                }
            },
            error: function () {
                alert("操作出错，请联系管理员！");
    ...
    function cancelReply(replyIds) {
        var path = "${BasePath}/meiyue/topic/recoverReply?params=" + replyIds + "&type=${type}&itemId=${itemId}";
        $.ajax({
            type: "POST",
            url: path,
            datatype: "json",
