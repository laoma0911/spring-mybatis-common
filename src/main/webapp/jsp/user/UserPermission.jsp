				...
				<div class="span12">
					...
					<div class="block">
						<div class="navbar navbar-inner block-header">
							<div class="muted pull-left">用户权限-${editingUser.name}</div>
							<input type="hidden" name="t_uid" value="${editingUser.id}" id="t_uid" />
						</div>
										...
										<table class="table table-bordered" style="padding:0;margin:0; border:0">
											...
											<tr style="border:0;">
												...
												<td class="Neitr" style="width:30px;text-align: center;border-left:0;">
													<input type="checkbox" style="margin-top:-2px;" class="single tr_nei_all_check qufen_${category.id}" value="${category.id}_${privilege.id}">
												</td>
												<td style="border-left:none;">
													...
													<c:if test="${privilege.id==operation.privilegeId}">
														<span class="t_operate">
															<c:if test="${category.id==privilege.categoryId}">
															<input type="checkbox" style="margin-top:-2px;" name="operationIds"	class="all_check checkval_${category.id} checkval_${category.id}_${privilege.id}" id="checks_${category.id}_${privilege.id}_${operation.id}" value="${category.id}_${privilege.id}_${operation.id}" >
															</c:if>
															<label class="checks" for="checks_${category.id}_${privilege.id}_${operation.id}">${operation.name}</label>
														</span>
													</c:if>
													</c:forEach>
												</td>
